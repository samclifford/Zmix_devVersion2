...
PostProc_mvn_Oct15<-function( Grun,  mydata ,Propmin=0.05, isSim=FALSE, simlabel="sim", savelabel="PPplot"){
     		require(wq)
	...
	#for each K0:
		#for ( .K0 in 1:length(K0)){
		 .K0<-which.max(p_vals$PropIters)  #most likely group only
		GrunK0<-Grun
		# split data by K0 (the most likely)
